...
- Still required before the 0017 checklist is closed: simultaneous status calls on a committed test order and live `pnpm dev` checkout/consent checks against T09. The local runtime could not reach the Supabase API during this session. Do not mark these browser checks complete from SQL or unit checks.

...
export default function BottomNav({ pathname, storeSlug }: { pathname: string; storeSlug: string }) {
  ...
  return <nav aria-label="Storefront mobile navigation" className="md:hidden fixed bottom-0 w-full z-50 min-h-[var(--storefront-mobile-nav-height)] rounded-t-xl bg-surface-container-low shadow-[0_-4px_30px_rgba(24,35,26,0.05)] border-t border-outline-variant/10 pb-safe"><div className="flex justify-around items-center px-2 py-3">{links.map(([href, label]) => <a href={href} aria-current={pathname === href ? 'page' : undefined} className={`touch-target flex flex-col items-center justify-center p-3 rounded-full transition-colors ${pathname === href ? active : inactive}`} key={href}><span className="whitespace-nowrap font-label-sm text-label-sm">{label === 'Bag' && hydrated && count > 0 ? `${label} (${count})` : label}</span></a>)}</div></nav>;
}
